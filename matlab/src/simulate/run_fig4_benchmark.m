...
% A run is provenance-bound only when live Git metadata is actually available.
% Downloaded source ZIPs have no .git directory and must remain unbound.
is_git_bound = prov.git_in_repo && ...
    ~isempty(regexp(prov.git_commit, '^[0-9a-f]{40}
...
summary.git_in_repo = prov.git_in_repo;
...
for k = 1:nCond
    ...
    qcStruct.provenance_status = provenance_status;
    qcStruct.git_in_repo = prov.git_in_repo;
...
manifest.git_in_repo = prov.git_in_repo;
manifest.provenance_status = provenance_status;
...
, 'once')) && ...
    islogical(prov.git_dirty);
provenance_status = ternary(is_git_bound, 'provenance_bound', ...
    'unbound_git_metadata_unavailable');
