...
%     manifests_wellformed      every results/<run_id>/manifest.json has all
%                               required provenance fields
%     evidence_guards           docs/evidence_levels.json pending/false
...
ip = inputParser;
d0 = fileparts(mfilename('fullpath'));            % .../matlab/simulate
root = fileparts(fileparts(d0));
...
% --- new-run manifests --------------------------------------------------
mandirs = arrayfun(@(d) fullfile(root, 'results', d.name), ...
    dir(fullfile(root, 'results', 'b1_*')), 'UniformOutput', false);
...
if nman == 0
    C{end+1} = mkCheck('manifests_wellformed', true, 'no provenance-bound runs yet (results/b1_*)');
else
    C{end+1} = mkCheck('manifests_wellformed', ok, sprintf('%d manifest(s) checked; %s', nman, det));
end

% --- evidence-level guards ---------------------------------------------
evpath = fullfile(root, 'docs', 'evidence_levels.json');
...
if ev.evidence_levels.equation_level_tests_passed == true
    ...
    C{end+1} = mkCheck('tests_evidence_log', ~isempty(dlogs), ...
        sprintf('%d test log file(s) under logs/ (AI-written tests, not human review)', ...
        numel(dlogs)));
end
