...
matlab -batch "addpath('matlab/src/simulate','matlab/generated'); run_fig4_benchmark"
# optional (requires data/raw/literature/R01/mavelli2015_fig4_page14_300dpi.png; regenerate with
#   pdftoppm -png -r 300 -f 14 -l 14 references/R01_Mavelli2015/A_Simple_Protein_Synthesis_Model_PURE_normalized.pdf <outdir>/fig4hi):
matlab -batch "addpath('matlab/src/simulate'); digitize_fig4"
```

Outputs: `results/baselines/b1_mavelli2015/DNA_{0p34,1p7,6p8}nM/{trajectory.csv,rates.csv,qc.json}`,
...
raster Fig. 4 (`matlab/tools/digitization/digitize_fig4.m`) reports [a]-panel deviations ≤ 1.8 % and
...
Seven items registered in `docs/project/benchmark_registry.md` (L1–L7); **none blocks reproduction**:
...
`reproduced = true` is justified **per evidence level** (see `docs/project/evidence_levels.json`):
