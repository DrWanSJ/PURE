...
`data/processed/fig4_digitized_validation_status.json`) with a **human,
...
The automated digitizer (`matlab/simulate/digitize_fig4.m`) must decide, at
...
1. **Hide the simulation.** Close all files under `results/` (both legacy
   `results/literature_reference/` and any `results/<run_id>/`), do not read
   `fig4_digitized_comparison.csv`, do not read the digitized CSVs under
   `data/processed/`, and do not read the numbers quoted in
   `docs/benchmark_v0.md` Section 5 before the reading is complete.
   The auditor must fill `simulation_hidden_during_reading = true` honestly.
2. **Print or open the source raster** at high zoom:
   `data/raw/mavelli2015_fig4_page14_300dpi.png` (300 dpi render of page 14
   of `A_Simple_Protein_Synthesis_Model_PURE_normalized.pdf`, sha256-locked
...
7. **Record the outcome** in `docs/audit_status.md` and
   `docs/evidence_levels.json`
