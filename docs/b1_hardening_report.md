...
- **A — baseline freeze:** annotated tag `ai-b1-baseline` ("AI-generated B1
  ...
  `docs/audit_status.md` records baseline facts (single root commit, not
  human-audited, no retroactive provenance) and `docs/evidence_levels.json`
  splits the single `reproduced=true` into explicit evidence layers with
  locked pending/false values.
- **B — real provenance for new runs:** `matlab/provenance/`
  ...
  `run_fig4_benchmark` now writes `results/<run_id>/manifest.json` for every
  new run; the legacy directory `results/literature_reference/` is frozen and
...
- **D — Fig. 4 circularity fixed:** `digitize_fig4.m` keeps its algorithm but
  ...
  `data/processed/fig4_digitized_validation_status.json` records this;
  blind human protocol (`docs/manual_fig4_audit_protocol.md`) + template
...
- **E — evidence levels:** `docs/evidence_levels.json` with
...
- **F — guards:** `matlab/simulate/release_preflight.m` (git/HEAD/dirty,
...
- **H — mutation protocol:** `docs/mutation_test_protocol.md` (M1–M5 with
...
- **I — licensing register:** `docs/licensing_review.md` — tracked publisher
...
- The legacy numerical results (`results/literature_reference/*.csv` values).
...
# -> logs/20260918_130736_tests.log: 18/18 unit tests, smoke passed, preflight run

# provenance-bound benchmark run:
matlab -batch "addpath('matlab/simulate','matlab/generated','matlab/provenance'); run_fig4_benchmark('RunId', 'b1_fig4_hardening_20260918')"
# -> results/b1_fig4_hardening_20260918/manifest.json (git fccf621…, dirty=false)

# numeric identity check vs legacy:
for d in DNA_0p34nM DNA_1p7nM DNA_6p8nM; do
  diff -q "results/literature_reference/$d/trajectory.csv" "results/b1_fig4_hardening_20260918/$d/trajectory.csv"
  diff -q "results/literature_reference/$d/rates.csv" "results/b1_fig4_hardening_20260918/$d/rates.csv"
done   # -> all IDENTICAL
```
...
`results/literature_reference/` values were produced before the provenance
...
4. Licensing decisions for the tracked publisher files (`docs/licensing_review.md`).
