...
| --- | --- | --- | --- |
| `A Simple Protein Synthesis Model for the PURE System Operation.pdf` | full published paper (Springer Nature, *Bulletin of Mathematical Biology* 77:1185–1212, © Society for Mathematical Biology 2015), original, password-protected | `7082156c…c93238a3` | redistributed in the repository as-is |
| `A_Simple_Protein_Synthesis_Model_PURE_normalized.pdf` | same paper, decrypted/normalized (Ghostscript) copy | `838d1f5d…6b4bb7d0` | created to enable page-by-page audit and the Fig. 4 digitization |
| `A Simple Protein Synthesis Model for the PURE System Operation.md` | full-text transcription/conversion of the same paper | `b47bb4e0…37af8` | contains the complete body text incl. all equations/tables |
| `data/raw/mavelli2015_fig4_page14_300dpi.png` | 300-dpi render of page 14 (contains Fig. 4) | tracked | derivative of the publisher's figure; kept so the digitization is auditable |
| `data/processed/fig4_digitized.csv`, `…_all_clusters.csv` | digitized coordinates of Fig. 4 calculated curves | tracked | derived data from the publisher's figure |
...
2. **remove from future commits** — `git rm --cached <file>` + `.gitignore`
   ...
   `pdftoppm -png -r 300 -f 14 -l 14 A_Simple_Protein_Synthesis_Model_PURE_normalized.pdf`.
3. **purge history** — `git filter-repo` (or BFG) + force-push; coordinate
   ...
   SHA recorded in `docs/audit_status.md` would change). This is the most
