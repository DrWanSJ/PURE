function created = drive_pnas2017_aa_v1_formal(runid)
%DRIVE_PNAS2017_AA_V1_FORMAL  Execute the registered v1r1 formal runs.
%
%Drives scripts/run_pnas2017_aa_v1_formal.m over the 13 registered run
%configs (run_registry.json), one pass, in registry order.  Per-run output
...
%
%created = drive_pnas2017_aa_v1_formal()        % all remaining runs
%created = drive_pnas2017_aa_v1_formal('RED-S3') % a single registered run
%
%This driver changes no numerics: it is the registered executor loop.

ROOT = fileparts(fileparts(mfilename('fullpath')));
FORMAL = fullfile(ROOT, 'results', 'pnas2017_reference', '2026-09-25_aa_v1_formal');
...
if isstruct(reg.runs) && numel(reg.runs) > 1
    ...
    toks = regexp(raw, '"(FULL-S\d|RED-S\d|RED-NEG1)"\s*:\s*\{', 'tokens');
    ids = [toks{:}];
