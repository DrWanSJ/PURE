#!/usr/bin/env python3
"""Build the v1r2 formal full-vs-reduced comparison (Phases 10-11).
...
Outputs (results/pnas2017_reference/2026-09-26_aa_v1r2_formal_comparison/):
...
FORMAL_V1R2 = os.path.join(ROOT, "results", "pnas2017_reference", "2026-09-26_aa_v1r2_formal")
...
OUTDIR = os.path.join(ROOT, "results", "pnas2017_reference",
                      "2026-09-26_aa_v1r2_formal_comparison")
...
def main():
    ...
    summary = {
        ...
        "generated_by": "scripts/build_pnas2017_aa_v1r2_formal_comparison.py",
...
def compare_pair(cid, scope, reg, hashes, tiers):
    full_csv = os.path.join(ROOT, reg["runs"]["FULL-%s" % cid]["trajectory"]
                            .replace("/", os.sep))
    red_csv = os.path.join(ROOT, reg["runs"]["RED-v1r2-%s" % cid]["trajectory"]
                           .replace("/", os.sep))
    ...
    fm = json.load(open(os.path.join(MANS, "FULL-%s.manifest.json" % cid),
                        encoding="utf-8"))
    rm = json.load(open(os.path.join(os.path.join(FORMAL_V1R2, "manifests"), "RED-v1r2-%s.manifest.json" % cid),
                        encoding="utf-8"))
    ...
    return {
        ...
        "red_config_sha256": reg["runs"]["RED-v1r2-%s" % cid]["config_sha256"],
...
def compare_negative(scope, reg, hashes, tiers):
    exp = reg["runs"]["RED-v1r2-NEG1"].get("expected_verdict", "FAIL")
    red_csv = os.path.join(ROOT, reg["runs"]["RED-v1r2-NEG1"]["trajectory"]
                           .replace("/", os.sep))
    ...
    try:
        ...
        rm_path = os.path.join(os.path.join(FORMAL_V1R2, "manifests"), "RED-v1r2-NEG1.manifest.json")
